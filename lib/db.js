...
// Migrate: add content_hash column (for existing databases)
{
  const cols = db.prepare('PRAGMA table_info(songs)').all().map((c) => c.name);
  if (!cols.includes('content_hash')) {
    db.exec('ALTER TABLE songs ADD COLUMN content_hash TEXT DEFAULT NULL');
  }
}
db.exec('CREATE INDEX IF NOT EXISTS idx_songs_user_hash ON songs(user_id, content_hash)');

// Backfill content_hash for rows created before the column existed
{
  const { computeSongHash } = require('./songHash');
  const rows = db.prepare('SELECT id, content FROM songs WHERE content_hash IS NULL').all();
  if (rows.length) {
    const upd = db.prepare('UPDATE songs SET content_hash = ? WHERE id = ?');
    const backfill = db.transaction(() => {
      for (const r of rows) upd.run(computeSongHash(r.content), r.id);
    });
    backfill();
  }
}
